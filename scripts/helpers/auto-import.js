...
hexo.extend.helper.register('autoImport', function (type) {
    switch (type) {
        case 'js':
            ...
            const js = tree.filter((file) => require$$1.extname(file) === '.js');
            const output = _(js)
...
(function getDirectoryTree(dirPath) {
    const dir = lib.readdirSync(dirPath);
    _.forEach(dir, (name) => {
        const deepPath = require$$1.join(dirPath, name);
        const stat = lib.statSync(deepPath);
        if (stat.isDirectory()) {
    ...
    return tree;
})(require$$1.resolve(__dirname, '../../source/libs'));
